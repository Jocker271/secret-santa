...
def get_persons():
    with open(f'{os.path.dirname(__file__)}\\{config.INPUT_FILE}', newline='') as csvfile:
...
def get_matches(persons, loop_count=0):
    ...
    for x in persons:
        ...
        matchable = [w['name'] for w in persons if w['name'] not in not_matching]
        if len(matchable) >= 1:
...
def send_mails(persons, matches, template):
    try:
        ...
        server.ehlo()
        server.starttls()
        server.login(config.MAIL_ACCOUNT_USERNAME, config.MAIL_ACCOUNT_PASSWORD)
        for wichtel in persons:
            ...
            server.sendmail(config.MAIL_ACCOUNT_USERNAME, mail_to, msg.as_string())
        server.quit()
